...
const ruleProviders = {
  ...
  "Emby": {
    "type": "http",
    "behavior": "classical",   // 列表包含 DOMAIN/DOMAIN-SUFFIX/IP-CIDR
    "format": "text",
    "interval": 86400,
    "url": "https://raw.githubusercontent.com/higfy5/clash/main/emby.list",
...
function parseCountries(config) {
  ...
  // 构建地区正则表达式（去除 (?i) 以免 JS RegExp 报错）
  const compiledRegex = {};
  for (const [country, meta] of Object.entries(countriesMeta)) {
    const p = String(meta.pattern || "").replace(/^\(\?i\)/, "");
    compiledRegex[country] = new RegExp(p, 'i');
  }

  for (const proxy of proxies) {
    const name = proxy.name || '';
    if (ispRegex.test(name)) continue; // 过滤 ISP/落地
...
function buildProxyGroups({
  ...
  // 将地区分组插入到最终分组列表前部（在“手动选择”之前更直观）
  const idx = groups.findIndex(g => g && g.name === "手动选择");
  const head = groups.slice(0, Math.max(0, idx));
  const tail = groups.slice(Math.max(0, idx));
  return [...head, ...countryProxyGroups, ...tail];
}
...
function main(config = {}) {
  ...
  const countryList = countryInfo.map(i => i.country);
  const countryProxyGroups = buildCountryProxyGroups(countryList);
  const finalProxyGroups = buildProxyGroups({
    countryList,
    countryProxyGroups,
    lowCost,
    defaultProxies,
